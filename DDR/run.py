...
def save_store(path, num, store):
    filename = '%s_store.json' % (num)
    filepath = os.path.join(path, filename)
    try:
        json.dump(store, open(filepath, "wb"))
        print 'saved model in %s' % (filepath,)
    except Exception, e:
        print 'Error: Writing model fails: %s' % (filepath,)
        print e

def simulation_epoch_store(ep,simulation_epoch_size, train=False):
    global store_kb_list
    successes = 0
    cumulative_reward = 0
    intrinsic_reward = 0
    cumulative_turns = 0
    ...
    for episode in xrange(simulation_epoch_size):
        ...
        n_list = []
        nl_list = []
        id = random.randint(0,len(all_goal_set)-1)
        goal = all_goal_set[id]
        u_nl = dialog_manager.initialize_episode_store(selected_goal=goal)
        nl_list.append(u_nl)
        episode_over = False
        while (not episode_over):
            episode_over, reward, n_old, n_new, a_nl, u_nl = dialog_manager.next_turn_store()
            ...
            if episode_over:
                ...
                if reward > 0:
                    ...
                    s_or_f = 0
                store_this['kb_list'] = n_list
                store_this['s_or_f'] = s_or_f
                store_this['episode'] = ep
                store_this['context'] = nl_list
                store_kb_list[id].append(store_this)
                cumulative_turns += dialog_manager.state_tracker.turn_count
            if train and step % 1 == 0:
                err, i_r = agent.train(batch_size, 1, dialog_manager.state_index_store)
...
def run_episodes(count, status):
    ...
    for episode in xrange(count):
        ...
        if (agt == 9 or agt == 12 or agt == 13) and params['trained_model_path'] == None:
            ...
            # simulation_res = simulation_epoch_train(2, train=True)
            simulation_res = simulation_epoch_store(1, train=True)
            ...
            # simulation_res = simulation_epoch_store(episode,simulation_epoch_size, train=False)
            ...
            if episode % save_check_point == 0 and params['trained_model_path'] == None: # save the model every 10 episodes
                ...
                save_store(params['write_model_dir'], num, store_kb_list)
