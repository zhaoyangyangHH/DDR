...
class RuleRestaurantSimulator(UserSimulator):
    ...
    def _sample_goal(self, goal_set):
        ...
        # self.goal_id = random.randint(0, len(self.start_set[self.learning_phase]) - 1)
        # sample_goal = self.start_set[self.learning_phase][self.goal_id]
    ...
    def back(self):
        ...
        self.episode_over = copy.deepcopy(self.episode_over_copy)
        self.dialog_status = copy.deepcopy(self.dialog_status_copy)
        self.constraint_check = copy.deepcopy(self.constraint_check_copy)

    def store(self):
        self.state_store = copy.deepcopy(self.state)
        self.episode_over_store = copy.deepcopy(self.episode_over)
        self.dialog_status_store = copy.deepcopy(self.dialog_status)
        self.constraint_check_store = copy.deepcopy(self.constraint_check)

    def recover(self):
        self.state = copy.deepcopy(self.state_store)
        self.episode_over = copy.deepcopy(self.episode_over_store)
        self.dialog_status = copy.deepcopy(self.dialog_status_store)
        self.constraint_check = copy.deepcopy(self.constraint_check_store)

    def next(self, system_action):
        """ Generate next User Action based on last System Action """
        self.state_copy = copy.deepcopy(self.state)
        self.episode_over_copy = copy.deepcopy(self.episode_over)
        self.dialog_status_copy = copy.deepcopy(self.dialog_status)
        self.constraint_check_copy = copy.deepcopy(self.constraint_check)
